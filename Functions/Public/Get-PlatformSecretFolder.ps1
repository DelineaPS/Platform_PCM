###########
#region ### global:Get-PlatformSecretFolder # CMDLETDESCRIPTION : Gets Secret Server Folders from the Platform Tenant :
###########
function global:Get-PlatformSecretFolder
{
    <#
    .SYNOPSIS
    Gets Secret Server Folders from Secret Server.

    .DESCRIPTION
    This function will search for Secret Server folders based on the provided input. This function 
    will return the special PlatformSecretFolder class object.
    ...
    .OUTPUTS
    This function outputs a PlatformSecretFolder class object.

    .EXAMPLE
    C:\PS> Get-PlatformSecretFolder
    Finds all Secret Server Folders visible to the user.

    .EXAMPLE
    C:\PS> Get-PlatformSecretFolder -Name "AAB"
    Finds all Secret Server Folders with "AAB" in the name.

    .EXAMPLE
    C:\PS> Get-PlatformSecretFolder -ID 599
    Finds the Secret Server Folder with the Folder ID of 599.

    .EXAMPLE
    C:\PS> Get-PlatformSecretFolder -ParentFolderId 599
    Recursively finds all child folders under the Secret Server Folder with the Folder Id 599.

    .EXAMPLE
    C:\PS> Get-PlatformSecretFolder -ParentFolderId 599 -LimitToDirectDescendents
	...
	.EXAMPLE
    C:\PS> Get-PlatformSecretFolder -OnlyIncludeRootFolders
    ...
    else # otherwise process the results
    {
        $AllData = $basequery  | Foreach-Object -Parallel {
			...
			Try
			{
				# create a new Platform Secret Folder object
				$folder = New-Object PlatformSecretFolder -ArgumentList ($query)
				# add it to our temporary returner object
				$obj | Add-Member -MemberType NoteProperty -Name Folders -Value $folder
			}
			Catch
			{
				# if an error occurred during New-Object, create a new PlatformPCMException and return that with the relevant data
				$e = New-Object PlatformPCMException -ArgumentList ("Error during New PlatformSecretFolder object.")
		...
		ForEach-Object -Begin { $i = 0 } -Process { 
			...
			$i++
			# update progress bar
			Write-Progress -Activity "Getting Folders" -Status ("{0} out of {1} Complete" -f $i,$basequery.Count) -PercentComplete $Completed -CurrentOperation ("Current: [{0}]" -f $_.folderName)
